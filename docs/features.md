...
- **도트(픽셀) 아트**: 고양이(고등어·치즈·삼색 코숏)·은화·가구를 문자 매트릭스→SVG로 렌더(다크모드 자동 대응). **고양이 6종 전부(고등어·치즈·삼색·하양·턱시도·까망) PixelLab PNG 스프라이트 시트(288×48 6프레임)로 CSS `steps()` 걷기 + 쉴 때 정지 4방향(앞/뒤/좌/우) 무작위 보기**(`PET_SPRITES`/`catActorHTML`, 시트 없는 동물은 SVG 매트릭스로 폴백). 하양은 export에 옆보기(east) 걷기가 없어 정면(south) 걷기 시트로 구성(걸을 때 정면). 새 동물 zip 처리 규칙은 [pet-asset-pipeline.md](pet-asset-pipeline.md).
- **전역 dock**: 모든 화면 상단에 얇은 스트립으로 은화 잔액 + 걸어다니는 고양이. `prefers-reduced-motion`·백그라운드 시 애니메이션 정지.
- **알뜰샵 시트**(더보기 → 알뜰샵, 아이콘=은화): **홈**(방 뷰 + 걷는 고양이 + 배치 가구, 보유 고양이 탭해 최대 3마리 집에 내보내기) / **상점**(고양이 3종·가구 3종, 가구 수량 허용) / **배치**(12×12 격자, 캣타워·스크래처는 2×2칸 점유·겹침 방지, 드래그로 이동, 배치된 가구 탭 시 회수/판매 메뉴) / **미션**(일일+주간).
- **주간 미션**: 5일 이상 기록(+20)·리포트 확인(+10), KST 월요일 초기화·멱등.
- **dock**: 얇은 스트립(탭 시 알뜰샵) ↔ 숨김(더보기 설정에서 토글).
- **고양이 6종**(고등어·치즈·삼색·까망·하양·턱시도), **가구 4종**(방석·밥그릇·캣타워·스크래처), **벽지 6종**(기본·하늘·벚꽃·민트·별밤·살구, 상점 벽지 탭에서 구매·적용). 상단 dock은 **웹캠 방**(정면 뷰)으로 벽지 배경 + 배치 가구(배경) + 걷는 고양이를 함께 보여줌. 가구는 **원근**을 적용해 앞(행이 작을수록)일수록 크고 아래, 뒤일수록 작고 위로 보이며 앞 가구가 뒤 가구를 가림.
- 고양이는 걷기 외에 **앉기·식빵·낮잠 포즈**(SVG 고양이) 또는 **멈춰 앞/뒤/좌/우 보며 쉬기**(스프라이트 고양이), 개별적으로 속도·방향·유휴가 랜덤이라 제각기 움직이며, 배치된 **가구로 다가가 종류별로 반응**(밥그릇=먹기·방석=식빵·캣타워=낮잠·스크래처=앉기).
