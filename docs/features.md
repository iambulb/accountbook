...
- **알뜰샵 시트**(더보기 → 알뜰샵, 아이콘=은화): **홈**(방 뷰 + 걷는 고양이 + 배치 가구, 보유 고양이 탭해 집에 내보내기 — **활성 슬롯 기본 3 + 잠금 4번째 슬롯을 금화 100으로 확장**) / **상점**(고양이·가구·**소비**(사료·물)·벽지·이벤트 탭, 가구 수량 허용) / **배치**(12×12 격자, 캣타워=3×6·스크래처=2×2칸 점유·겹침 방지, 드래그로 이동, 배치된 가구 탭 시 회수/판매 메뉴) / **미션**(일일+주간).
...
- **고양이 이름 짓기**: 홈 "우리집 고양이" 칩의 **✎ 버튼**으로 고양이별 이름을 지을 수 있음(최대 12자, `owned.cats[id].name`). 지은 이름도 **등급 색**(한정=무지개)으로 표기됨.
