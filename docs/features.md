...
- **알뜰샵 시트**(더보기 → 알뜰샵, 아이콘=은화): **홈**(방 뷰 + 걷는 고양이 + 배치 가구, 보유 고양이 탭해 집에 내보내기 — **활성 슬롯 기본 3 + 잠금 4번째 슬롯을 금화 100으로 확장**) / **상점**(고양이·가구·**소비**(사료·물)·벽지·이벤트 탭, 가구 수량 허용 — **고양이 카드를 탭하면 선택**되어 강조되고 썸네일이 옆으로 걷는 스프라이트로 미리보기됨 — 이름 오른쪽 분류 태그는 **품종이 아니라 종(species) 라벨**(고양이/강아지/토끼, `SPECIES_LABEL`)) / **배치**(12×12 격자, 캣타워=3×6·스크래처=2×2칸 점유·겹침 방지, 드래그로 이동, 배치된 가구 탭 시 회수/판매 메뉴) / **미션**(일일+주간).
...
- **고양이 12종**(고등어·치즈·삼색·까망·하양·복슬이·턱시도·카오스·샴·벵갈·폴드·보라), **가구 6종**(방석·밥그릇·물그릇·캣타워·스크래처·화장실), **벽지 6종**(기본·하늘·벚꽃·민트·별밤·살구, 상점 벽지 탭에서 구매·적용). 고양이 상점 가격(은화)은 **등급·확률에 맞춰 산정**(일반50·고급100·희귀200·특별400·전설800·한정1500, `TIER_PRICE`가 `CAT_TIER` 단일 소스로 자동 산정 — 알 100은화+금화·환급 대비 흔한 등급은 싸게·희귀는 비싸게). 상단 dock은 **웹캠 방**(위에서 아래로 내려다보는 뷰, 바닥 넓게·벽지 좁게)으로 벽지 배경 + 배치 가구(배경) + 걷는 고양이를 함께 보여주며, **LIVE 배지는 좌측 상단**. 가구는 **원근**을 적용해 앞(격자 **아랫줄**)일수록 크고 아래, 뒤(격자 **윗줄**)일수록 작고 위로 보이며 앞 가구가 뒤 가구를 가림. **배치 탭**엔 현재 배치를 실제 방으로 보여주는 **미니 웹캠 프리뷰**가 있어 방향을 확인할 수 있음(격자 위=방 뒤, 아래=방 앞).
- 고양이는 걷기 외에 **앉기·식빵·낮잠 포즈**(SVG 고양이) 또는 **멈춰 앞/뒤/좌/우 보며 쉬기**(스프라이트 고양이), 개별적으로 속도·방향·유휴가 랜덤이라 제각기 움직이며, 배치된 **가구로 다가가 종류별로 반응하며 몇 초간 머무름**(캣타워=3층 중 한 층에 올라가 정면 보며 쉼·방석=위에 잠시 올라감·밥그릇=뒤에서 앉아 먹음·스크래처=옆에서 머무름).
