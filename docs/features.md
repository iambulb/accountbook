...
- **알뜰샵 시트**(더보기 → 알뜰샵, 아이콘=은화): **홈**(방 뷰 + 걷는 고양이 + 배치 가구, 보유 고양이 탭해 최대 3마리 집에 내보내기) / **상점**(고양이·가구·**소비**(사료·물)·벽지·이벤트 탭, 가구 수량 허용) / **배치**(12×12 격자, 캣타워=3×6·스크래처=2×2칸 점유·겹침 방지, 드래그로 이동, 배치된 가구 탭 시 회수/판매 메뉴) / **미션**(일일+주간).
- **주간 미션**: 5일 이상 기록(+20)·리포트 확인(+10), KST 월요일 초기화·멱등.
- **dock**: 얇은 스트립(탭 시 알뜰샵) ↔ 숨김(더보기 설정에서 토글).
- **고양이 7종**(고등어·치즈·삼색·까망·하양·턱시도·카오스), **가구 4종**(방석·밥그릇·캣타워·스크래처), **벽지 6종**(기본·하늘·벚꽃·민트·별밤·살구, 상점 벽지 탭에서 구매·적용). 상단 dock은 **웹캠 방**(정면 뷰)으로 벽지 배경 + 배치 가구(배경) + 걷는 고양이를 함께 보여줌. 가구는 **원근**을 적용해 앞(행이 작을수록)일수록 크고 아래, 뒤일수록 작고 위로 보이며 앞 가구가 뒤 가구를 가림.
...
- **🍚 다마고치 사료·물·배변 루프**: 상점 **소비 탭**에서 **사료·물**(각 1 은화, 배치 불가) 구매 → 홈 화면에서 **밥그릇·물그릇을 탭**하면 사료/물 1개를 써서 채워짐(이미지로도 밥·물이 채워짐). 채운 지 **3시간 뒤 그릇이 비워지고**, 비운 그릇 하나당 **화장실(펫 화장실)에 똥 1개**가 쌓임. **똥을 탭하면 +2 은화**(작은 획득 연출). 앱을 가끔 열어 상호작용하며 은화를 버는 컨셉. (밥그릇 기본 이미지는 빈 그릇)
