...
- **고양이 7종**(고등어·치즈·삼색·까망·하양·턱시도·카오스), **가구 4종**(방석·밥그릇·캣타워·스크래처), **벽지 6종**(기본·하늘·벚꽃·민트·별밤·살구, 상점 벽지 탭에서 구매·적용). 고양이 상점 가격(은화)은 **등급·확률에 맞춰 산정**(일반50·고급100·희귀200·특별400·전설800·한정1500, `TIER_PRICE`가 `CAT_TIER` 단일 소스로 자동 산정 — 알 100은화+금화·환급 대비 흔한 등급은 싸게·희귀는 비싸게). 상단 dock은 **웹캠 방**(정면 뷰)으로 벽지 배경 + 배치 가구(배경) + 걷는 고양이를 함께 보여주며, **LIVE 캠 배지는 방 하단**(아래에서 촬영하는 컨셉). 가구는 **원근**을 적용해 앞(행이 작을수록)일수록 크고 아래, 뒤일수록 작고 위로 보이며 앞 가구가 뒤 가구를 가림.
