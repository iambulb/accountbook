...
- **도트(픽셀) 아트**: 고양이(고등어·치즈·삼색 코숏)·은화·가구를 문자 매트릭스→SVG로 렌더(다크모드 자동 대응). **고등어·치즈·하양 고양이는 PixelLab PNG 스프라이트 시트(288×48 6프레임)로 CSS `steps()` 걷기 + 쉴 때 정지 4방향(앞/뒤/좌/우) 무작위 보기**, 나머지 고양이는 SVG 매트릭스 걷기(`PET_SPRITES`/`catActorHTML`). 하양은 export에 옆보기(east) 걷기가 없어 정면(south) 걷기 시트로 구성(걸을 때 정면). 새 고양이 zip 처리 규칙은 [pet-asset-pipeline.md](pet-asset-pipeline.md).
...
- **고양이 5종**(고등어·치즈·삼색·까망·하양), **가구 4종**(방석·밥그릇·캣타워·스크래처), **벽지 6종**(기본·하늘·벚꽃·민트·별밤·살구, 상점 벽지 탭에서 구매·적용). 상단 dock은 **웹캠 방**(정면 뷰)으로 벽지 배경 + 배치 가구(배경) + 걷는 고양이를 함께 보여줌. 가구는 **원근**을 적용해 앞(행이 작을수록)일수록 크고 아래, 뒤일수록 작고 위로 보이며 앞 가구가 뒤 가구를 가림.
