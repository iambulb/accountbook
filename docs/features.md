...
- **고양이 8종**(고등어·치즈·삼색·까망·하양·턱시도·카오스·샴), **가구 6종**(방석·밥그릇·물그릇·캣타워·스크래처·화장실), **벽지 6종**(기본·하늘·벚꽃·민트·별밤·살구, 상점 벽지 탭에서 구매·적용). 고양이 상점 가격(은화)은 **등급·확률에 맞춰 산정**(일반50·고급100·희귀200·특별400·전설800·한정1500, `TIER_PRICE`가 `CAT_TIER` 단일 소스로 자동 산정 — 알 100은화+금화·환급 대비 흔한 등급은 싸게·희귀는 비싸게). 상단 dock은 **웹캠 방**(위에서 아래로 내려다보는 뷰, 바닥 넓게·벽지 좁게)으로 벽지 배경 + 배치 가구(배경) + 걷는 고양이를 함께 보여주며, **LIVE 배지는 좌측 상단**. 가구는 **원근**을 적용해 앞(행이 작을수록)일수록 크고 아래, 뒤일수록 작고 위로 보이며(배치 맨 앞 행=바닥 앞끝, 맨 뒤 행=바닥 뒤끝까지 반영) 앞 가구가 뒤 가구를 가림. **배치 탭**엔 현재 배치를 실제 방으로 보여주는 **미니 웹캠 프리뷰**가 있어 방향을 확인할 수 있음.
