...
| 리포트 | 📊 | 월 요약·카테고리 도넛차트·6개월 추이 막대차트·**개인별 지출(용돈)/공동 지출 분리 비교** | `renderStats` |
| (가운데) **＋** | ➕ | 거래 추가/편집 시트 | `openTxSheet` |
| 자산 | 🏦 | 결제수단 목록·잔액·카드 실적·선불/포인트 요약·적금 목표 | `renderAssets` |
| 더보기 | ☰ | 설정·관리 메뉴 허브(+ 🐱 고양이집 진입) | `renderMore` |

상단바에는 **워크스페이스 칩**(현재 가계부 전환), **테마 토글**(🌙/☀️)이 있습니다. 상단바 아래에는 **고양이집 dock**(걸어다니는 도트 고양이 + 은화, 탭 시 고양이집)이 상시 노출됩니다.
...
- **은화 미션**: 일일 미션(오늘 거래 1건 기록 +5 / 출석 +2)을 KST 자정 기준으로 판정, **멱등 수령**(같은 날 중복 지급 불가). 출석은 진입 시 자동 수령.
- **도트(픽셀) 아트**: 고양이(고등어·치즈·삼색 코숏)·은화·가구를 문자 매트릭스→SVG로 렌더(다크모드 자동 대응, PNG 미사용).
- **전역 dock**: 모든 화면 상단에 얇은 스트립으로 은화 잔액 + 걸어다니는 고양이. `prefers-reduced-motion`·백그라운드 시 애니메이션 정지.
- **고양이집 시트**(더보기 → 고양이집, 아이콘=은화): **홈**(방 뷰 + 걷는 고양이 + 배치 가구, 보유 고양이 탭해 최대 3마리 집에 내보내기) / **상점**(고양이 3종·가구 3종, 가구 수량 허용) / **배치**(12×12 격자 스냅) / **미션**(일일+주간).
- **주간 미션**: 5일 이상 기록(+20)·리포트 확인(+10), KST 월요일 초기화·멱등.
- **dock 3모드**: 접힌 스트립 ↔ 인라인 확장(방) ↔ 숨김(더보기 설정에서 토글).
