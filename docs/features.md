...
- **이벤트(뽑기)**: 상점 이벤트 탭에서 **펫알**(→고양이)·**랜덤박스**(→가구) 각 100 은화. 펫알·랜덤박스 아트는 **무지개 물음표**가 그려진 달걀/상자 도트. 구매 시 화면 중앙 오픈 연출(펫알 3탭 균열, 랜덤박스 화려 오픈) → 등급(일반/고급/희귀/특별/전설/한정, 확률 60/20/15/3.8/1/0.2%) 랜덤 보상. 오픈 절정에서 **껍질 조각이 튀어 옆에 흩어지고**, 껍질 사이로 새어나오는 빛은 **등급색·등급이 높을수록 강하게**(한정=무지개), 특별↑은 검은 고양이 앞발이 톡 건드림. 열 때마다 **금화 1개**, 중복 고양이는 30 은화 환급.
