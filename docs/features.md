...
- **전역 dock**: 모든 화면 상단에 얇은 스트립으로 은화 잔액 + 걸어다니는 고양이. `prefers-reduced-motion`·백그라운드 시 애니메이션 정지. **캠/방에서 펫을 꾹 누르면(롱프레스 320ms) 집어서 좌우로 이동**할 수 있음(놓으면 그 자리에서 다시 배회, `petGrabDown`). **알뜰샵은 dock 좌상단 은화 배지를 탭**해 열며(누르는 press 애니메이션 후 열림, `coinTap`), 캠 빈 곳 탭은 아무 동작도 하지 않음.
