...
- **도트(픽셀) 아트**: 고양이(고등어·치즈·삼색 코숏)·은화·가구를 문자 매트릭스→SVG로 렌더(다크모드 자동 대응). **고양이 13종 전부(고등어·치즈·삼색·하양·복슬이·턱시도·까망·카오스·샴·벵갈·폴드·보라·초코) PixelLab PNG 스프라이트 시트(288×48 6프레임)로 CSS `steps()` 걷기**(`PET_SPRITES`/`catActorHTML`, 시트 없는 동물은 SVG 매트릭스로 폴백). **옆으로 이동할 땐 옆 걷기 시트만**(east 기준, 서쪽은 `scaleX(-1)`), **가만히 서 있거나 캣타워·방석 등에서 쉴 땐 정면(south) 정지 이미지**로 전환(정면으로 서 있는 시간을 길게). 걷기 애니메이션 주기는 이동 속도에 맞춰(`--wdur`/`walkDur`) 발이 바닥을 미끄러지지 않게(무빙워크 방지). 옆(east) 걷기 시트가 없는 export는 `frontWalk`로 **이동 중 east 정지스틸만 보여 정면 걷기를 방지**(현재 12종 전부 east 옆걷기 보유 — 해당 없음). 새 동물 zip 처리 규칙은 [pet-asset-pipeline.md](pet-asset-pipeline.md).
