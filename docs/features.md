...
- **도트(픽셀) 아트**: 고양이(고등어·치즈·삼색 코숏)·은화·가구를 문자 매트릭스→SVG로 렌더(다크모드 자동 대응). **고양이 11종 전부(고등어·치즈·삼색·하양·턱시도·까망·카오스·샴·벵갈·폴드·보라) PixelLab PNG 스프라이트 시트(288×48 6프레임)로 CSS `steps()` 걷기**(`PET_SPRITES`/`catActorHTML`, 시트 없는 동물은 SVG 매트릭스로 폴백). **옆으로 이동할 땐 옆 걷기 시트만**(east 기준, 서쪽은 `scaleX(-1)`), **가만히 서 있거나 캣타워·방석 등에서 쉴 땐 정면(south) 정지 이미지**로 전환(정면으로 서 있는 시간을 길게). 걷기 애니메이션 주기는 이동 속도에 맞춰(`--wdur`/`walkDur`) 발이 바닥을 미끄러지지 않게(무빙워크 방지). 하양은 export에 옆(east) 걷기가 없어(`frontWalk`) **이동 중엔 east 정지스틸로 옆을 보게** 처리(정면 걷기 방지). 새 동물 zip 처리 규칙은 [pet-asset-pipeline.md](pet-asset-pipeline.md).
- **전역 dock**: 모든 화면 상단에 얇은 스트립으로 은화 잔액 + 걸어다니는 고양이. `prefers-reduced-motion`·백그라운드 시 애니메이션 정지.
- **알뜰샵 시트**(더보기 → 알뜰샵, 아이콘=은화): **홈**(방 뷰 + 걷는 고양이 + 배치 가구, 보유 고양이 탭해 집에 내보내기 — **활성 슬롯 기본 3 + 잠금 4번째 슬롯을 금화 100으로 확장**) / **상점**(고양이·가구·**소비**(사료·물)·벽지·이벤트 탭, 가구 수량 허용 — **고양이 카드를 탭하면 선택**되어 강조되고 썸네일이 옆으로 걷는 스프라이트로 미리보기됨) / **배치**(12×12 격자, 캣타워=3×6·스크래처=2×2칸 점유·겹침 방지, 드래그로 이동, 배치된 가구 탭 시 회수/판매 메뉴) / **미션**(일일+주간).
