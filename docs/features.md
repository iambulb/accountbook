...
- **도트(픽셀) 아트**: 고양이(고등어·치즈·삼색 코숏)·은화·가구를 문자 매트릭스→SVG로 렌더(다크모드 자동 대응). **고등어·치즈·삼색·하양·턱시도 고양이는 PixelLab PNG 스프라이트 시트(288×48 6프레임)로 CSS `steps()` 걷기 + 쉴 때 정지 4방향(앞/뒤/좌/우) 무작위 보기**, 나머지(까망)는 SVG 매트릭스 걷기(`PET_SPRITES`/`catActorHTML`). 하양은 export에 옆보기(east) 걷기가 없어 정면(south) 걷기 시트로 구성(걸을 때 정면). 새 동물 zip 처리 규칙은 [pet-asset-pipeline.md](pet-asset-pipeline.md).
- **전역 dock**: 모든 화면 상단에 얇은 스트립으로 은화 잔액 + 걸어다니는 고양이. `prefers-reduced-motion`·백그라운드 시 애니메이션 정지.
- **알뜰샵 시트**(더보기 → 알뜰샵, 아이콘=은화): **홈**(방 뷰 + 걷는 고양이 + 배치 가구, 보유 고양이 탭해 최대 3마리 집에 내보내기) / **상점**(고양이 3종·가구 3종, 가구 수량 허용) / **배치**(12×12 격자 스냅) / **미션**(일일+주간).
