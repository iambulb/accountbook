...
- **도트(픽셀) 아트**: 고양이(고등어·치즈·삼색 코숏)·은화·가구를 문자 매트릭스→SVG로 렌더(다크모드 자동 대응). **고등어 고양이는 PixelLab PNG 스프라이트 시트(288×48 6프레임)로 CSS `steps()` 걷기**, 나머지 고양이는 SVG 매트릭스 걷기(`CAT_SPRITES`/`catActorHTML`).
...
- **dock**: 얇은 스트립(탭 시 고양이집) ↔ 숨김(더보기 설정에서 토글).
...
- 고양이는 걷기 외에 **앉기·식빵·낮잠 포즈**, 개별적으로 속도·방향·유휴가 랜덤이라 제각기 움직이며, 배치된 **가구로 다가가 종류별로 반응**(밥그릇=먹기·방석=식빵·캣타워=낮잠·스크래처=앉기).
