...
- **전역 dock**: 모든 화면 상단에 얇은 스트립으로 은화 잔액 + 걸어다니는 고양이. `prefers-reduced-motion`·백그라운드 시 애니메이션 정지. **캠/방에서 펫을 꾹 누르면(롱프레스 320ms) 집어서 좌우로 이동**할 수 있음(놓으면 그 자리에서 다시 배회, `petGrabDown`). 짧은 탭은 기존대로 알뜰샵 열기.
