...
- **도트(픽셀) 아트**: 고양이(고등어·치즈·삼색 코숏)·은화·가구를 문자 매트릭스→SVG로 렌더(다크모드 자동 대응). **고등어·치즈 고양이는 PixelLab PNG 스프라이트 시트(288×48 6프레임)로 CSS `steps()` 걷기 + 쉴 때 정지 4방향(앞/뒤/좌/우) 무작위 보기**, 나머지 고양이는 SVG 매트릭스 걷기(`CAT_SPRITES`/`catActorHTML`). 새 고양이 zip 처리 규칙은 [cat-asset-pipeline.md](cat-asset-pipeline.md).
...
- **고양이 5종**(고등어·치즈·삼색·까망·하양), **가구 4종**(방석·밥그릇·캣타워·스크래처), **벽지 6종**(기본·하늘·벚꽃·민트·별밤·살구, 상점 벽지 탭에서 구매·적용). 상단 dock은 **웹캠 방**으로 벽지 배경 + 배치 가구(배경) + 걷는 고양이를 함께 보여줌.
...
- **이벤트(뽑기)**: 상점 이벤트 탭에서 **펫알**(→고양이)·**랜덤박스**(→가구) 각 100 은화. 구매 시 화면 중앙 오픈 연출(펫알 3탭 균열, 랜덤박스 화려 오픈) → 등급(일반/고급/희귀/특별/전설/한정, 확률 60/20/15/3.8/1/0.2%) 랜덤 보상. 열 때마다 **금화 1개**, 중복 고양이는 30 은화 환급.
