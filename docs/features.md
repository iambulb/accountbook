...
- **알뜰샵 시트**(더보기 → 알뜰샵, 아이콘=은화): **홈**(방 뷰 + 걷는 고양이 + 배치 가구, 보유 고양이 탭해 집에 내보내기 — **활성 슬롯 기본 3 + 잠금 4번째 슬롯을 금화 100으로 확장**) / **상점**(고양이·가구·**소비**(사료·물)·벽지·이벤트 탭, 가구 수량 허용 — **고양이 카드를 탭하면 선택**되어 강조되고 썸네일이 옆으로 걷는 스프라이트로 미리보기됨 — 이름 오른쪽 분류 태그는 **품종이 아니라 종(species) 라벨**(고양이/강아지/토끼, `SPECIES_LABEL`)) / **배치**(12×12 격자, 캣타워=3×6·스크래처=2×2·화장실=2×1·펫하우스=3×3칸 점유·겹침 방지, **꾹 눌러(롱프레스) 드래그로 배치·이동**(화면 스크롤과 겹치지 않도록) — 짧게 탭하면 팔레트=선택·배치된 가구=회수/판매 메뉴) / **미션**(일일+주간).
