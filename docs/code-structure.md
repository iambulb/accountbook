...
| 개발자 모드 | `isDev`·`devOn`·`toggleDevMode`·`devCfg`/`saveDevCfg`·`openDevGacha`·`openDevPetManager`(펫 추가/수정/삭제 `submitDevPet`·`deletePetSoft`, 가챠 연출 펫 지정 `setGachaFxSlot`/`gachaFxSlotOf`)·`openDevDataTools`(데이터 정리: `migrateCatalogArtOnce` 구 인라인 아트 분리 이전 + `exportPetStatic` 런타임→정적 승격 내보내기)·`loadGachaFx`(config/gachaFx 구독) (cats); 진입 `openDevModeSheet`(views, 더보기 그리드 무지개알 타일) |
