...
| `js/util.js` | ~60 | **순수 계산 유틸**(돈/통화/정산): `won`·`fmtComma`·`parseAmount`·`curInfo`·`fmtForeign`·`krwFromForeign`·`sumByCurrency`·`computeSettleAmounts`·`personKey`·`addDays`·`nextDue`·`dueDiffDays`·`CURRENCIES`. 브라우저 전역 + Node `module.exports` 듀얼 → `test/`에서 단위 테스트(13케이스). |
