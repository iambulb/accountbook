...
| 내보내기 | `exportCSV` |
| **할일(투두)** | `renderTodoList`/`renderTodoCalendar`/`renderTodoDone`·`todoRow`/`todoDueBadge`·`scopedTodos`(개인=`myTodos`·친구=`friendTodos`·그룹=`ws`)/`allTodos`/`todoDbRef`·`todoTabSeg`/`setTodoPersonalTab`(개인 컨텍스트=[내 할일|친구들], 그룹은 세그먼트 없음)·CRUD `openTodoEdit`/`saveTodo`/`deleteTodo`/`toggleTodo`·더보기 시트 `openTodoReport`/`openRepeatTodos`·PB연동 `pbTodoSummaryHtml`. 상세 [features-todo.md](features-todo.md) |
