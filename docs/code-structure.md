...
| 개발자 모드 | `isDev`·`devOn`·`toggleDevMode`·`devCfg`/`saveDevCfg`·`openDevGacha`·`openDevPetManager`(행 `devPetRowHtml`+액션 `devPetActionsHtml` 분리, `devSelectPet`=행·`#pmActions`만 부분 갱신; 펫 추가/수정/삭제 `submitDevPet`·`deletePetSoft`; 가챠 연출 펫 지정 `setGachaFxSlot`/`gachaFxSlotOf`/`gachaFxSlotDesc`, 미리보기 `devPreviewGachaFx`=3탭·리빌 없이 시퀀스만 재생·자동종료; 실전/미리보기 공용 시퀀스 예약 `fxCatSeqSchedule`; 발끝 정합=알 바닥 실측 `--floor`+걷는시트 여백 실측 `measureFxFoot`(`_measurePadUrl`)+사전측정 `prewarmGachaFxPads`)·`openDevDataTools`(데이터 정리: `migrateCatalogArtOnce`→확인 후 `_migrateCatalogArtOnce` + `exportPetStatic` 런타임→정적 승격)·`loadGachaFx`(config/gachaFx 구독) (cats); 진입 `openDevModeSheet`(views, 더보기 그리드 무지개알 타일) |
