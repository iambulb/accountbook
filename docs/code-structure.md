...
| `js/cats.js` | ~360 | 🐱 고양이집 — 은화 경제(`users/{uid}/game`: coins/owned/home/missions/progress/codes), 도트 아트(`pxSvg`+매트릭스, 고양이 5종 걷기2+정면+포즈 `catPose`(sit/loaf/sleep), 가구 4종), 단일 rAF 걷기 엔진(`buildActors`/`stepActors` 가구별 상호작용 `poseForItem`), 전역 dock(`#catdock` 스트립/숨김), 고양이집 시트(홈·상점·배치·미션), 프로모(`redeemCode`/`PROMO_CODES`), 카탈로그 상수 |
