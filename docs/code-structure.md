...
| **친구 집(펫캠)·좋아요** | `openFriendHome`(views, 캠+좋아요+오늘할일)·`likeFriendHome`·`friendChangedToday`. cats: `friendRoomHtml`/`mountFriendRoom`/`friendActiveCats`/`friendPlacedList`, `likeHome`/`homeLikeCount`/`likedTodayBy`/`watchMyLikes`(❤️ 하루1회·총합), `touchHome`(펫/가구 변경→`home.changedAt`). 엔진: `activeStage`가 `#frStage` 우선·`buildActors` 가구 소스 분기·`propMarkup(...,plain)` 정적. 아이콘 `heartSvg`/`peopleSvg`(`M_HEART`/`M_PEOPLE`) |
