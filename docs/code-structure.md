...
| **펫캠 PiP 미니 창(데스크톱)** | cats: `openPipCam`(Document Picture-in-Picture, 크롬·엣지 116+ — `pipSupported` 게이트라 모바일/TWA에선 버튼·코드 전부 휴면)·`renderPipProps`/`renderPipCats`(dock와 동일 서명 가드)·`_pipLayout`(가상 높이 `PIP_VH`=200 방을 창 크기에 transform:scale — 펫·가구 크기·원근 dock와 동일)·`_pipStatic`(핸들러 제거=시청 전용)·`_pipClosed`(pagehide 정리: `_stageW`/`_petX`/`_petPose` 등 무대 캐시 삭제+rAF 메인 복구)·`syncPipTheme`(core `applyTheme` 훅). 엔진: `activeStages`가 `_pip.stage` 편입(탭 숨김 시 PiP만 스텝), `catLoop`/`startCatLoop`가 `_engWin()`(PiP 열림 시 PiP 창)의 rAF로 스케줄+`Date.now()` 타이밍(창 간 rAF 원점 차 흡수) → 메인 탭이 백그라운드여도 미니 캠 애니 지속. 버튼 `pipBtnHtml`/`.cd-pip`(픽셀 아이콘 `pipSvg`/`M_PIP`) |
