...
- `migrationV3` 플래그로 중복 실행을 막는다(플래그가 있으면 `migrateLegacyIfNeeded`가 즉시 return → 이후 어떤 클라이언트도 레거시 루트를 읽지 않음).
- **2026-07 보안 강화**: `migrationV3`가 이미 설정돼(마이그레이션 완료) 아무도 레거시 루트를 안 읽으므로, **레거시 루트 컬렉션(`accounts`/`transactions`/`people`/`loans` 등 17종)을 규칙에서 전면 제거**해 크로스테넌트 읽기/쓰기 노출을 닫았다. 원본 루트 데이터는 이제 규칙상 **읽기 불가**로 보호되며(삭제 불필요), 원하면 콘솔에서 수동 삭제해도 된다.
