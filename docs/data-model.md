...
config/wallpaper/{id}     : { tier?, price? }                 // 🧱 벽지 전역 등급/가격 오버라이드. loadWallCfg 구독 → wallTierOf(effWallTier)·wallBuyPrice가 WALL_TIER/WALLPAPER_CATALOG.price에 병합. 특별↑=isGachaOnlyWall 자동 랜덤박스 전용. '기구물 관리' 벽지 탭(setWallTier/setWallPrice/resetWall)에서 편집(규칙=config 상위가 커버, 변경 불필요)
config/floor/{id}         : { tier?, price? }                 // 🟫 바닥 스킨 전역 등급/가격 오버라이드. loadFloorCfg 구독 → floorTierOf(effFloorTier)·floorBuyPrice가 FLOOR_TIER/FLOOR_CATALOG.price에 병합. 특별↑=isGachaOnlyFloor 자동 랜덤박스 전용. '기구물 관리' 바닥 탭(setFloorTier/setFloorPrice/resetFloor)에서 편집
