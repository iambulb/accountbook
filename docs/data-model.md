...
users/{uid}            : { name, email, photo(프로필 사진 base64 data URL), createdAt, activeWs, recentWs:{ ledger, todo },  // 🔀 모드별 최근 컨텍스트 wsId(가계부/할일 각각 마지막 사용처 — 개인 프로필=ws_{uid} 또는 그룹). 모드 토글 시 각자 복원. activeWs=마지막 활성(하위호환)
                           ...
                           game:{ 🐱 고양이집(개인 전역, 워크스페이스 무관)
                             ...
                             rbcoin,                                 // 🌈 무지개동전(2026-07 신설, 최대 999,999 MAX_RBCOIN) — 한정(exclusive) 등급 중복 획득 시 +1(전 가챠 경로, grantRbcoin). 소비=무지개알/무지개박스 1뽑 5개·10연 50개(openRainbow/openGachaTen)
                             rbcoinTotal,                            // 🌈 무지개동전 누적 획득(소비해도 안 줄어듦 — CS·어뷰즈 추적용, grantRbcoin이 함께 증가)
