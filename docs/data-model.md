...
### creditCards/{id}
`cardName`, `cardCompany`, `monthlyPerformanceTarget`, `performancePeriodType`(calendar_month/custom), `performanceStartDay`, `usageSameAsPerf`(이용(청구) 기간=실적 기간과 동일 — 기본/미설정=true), `usagePeriodType`(calendar_month/custom), `usageStartDay`, `billingDay`(결제일 1~31 — 카드 대금 자동 기록용), `billingAccountId`(대금 출금 계좌), `billingAutoSince`(자동 기록 켠 날 — 이전 결제일 소급 방지), `lastBilled`(마지막 자동 기록 회차 날짜 — 멱등 마커, 생성분을 지워도 재생성 안 함), `includePrepaidCharge`, `excludedCategories[]`, `defaultIncluded`, `visibility`, `memo`. 결제일+출금 계좌가 모두 있으면 `runCardBills`(core.js, 부팅 시)가 결제일에 직전 마감 청구(이용) 기간 사용액을 `transactions/{uid}/cardbill_{cardId}_{결제일}` 이체(transfer, `cardBillKey`·`billPeriodEnd` 마커)로 자동 기록.
