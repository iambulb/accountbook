...
### creditCards/{id}
`cardName`, `cardCompany`, `monthlyPerformanceTarget`, `performancePeriodType`(calendar_month/custom), `performanceStartDay`, `usageSameAsPerf`(이용(청구) 기간=실적 기간과 동일 — 기본/미설정=true), `usagePeriodType`(calendar_month/custom), `usageStartDay`, `includePrepaidCharge`, `excludedCategories[]`, `defaultIncluded`, `visibility`, `memo`.
