...
| 탭 | 아이콘 | 화면 | 주요 함수 |
|---|---|---|---|
| 자산 | 🏦 | 결제수단 목록·잔액·카드 실적·선불/포인트 요약(**잔액 마이너스 시 자동충전**)·적금 목표 | `renderAssets` |
| 캘린더 | 📅 | 월 네비게이션 + 요약 + 달력 그리드(일별 수입/지출) / 거래내역 리스트 | `renderCalendar` |
| (가운데) **＋** | ➕ | 거래 추가/편집 시트 | `openTxSheet` |
| 리포트 | 📊 | 월 요약·카테고리 도넛차트(**항목 탭=그 카테고리 내역 시트**)·6개월 추이·**개인별/공동 지출 분리**·**통화별 지출**(해외통화 있을 때)·**전월비 급증 인사이트**·예산 임계 경고 | `renderStats`/`openRepCatTx` |
...
- **📅 지난달 카드 내역 보기(리포트)**: 리포트 탭의 **카드별** 섹션은 **월 네비(‹ ›)가 고른 달**을 따른다 — 그 달 카드별 결제액·건수와 실적 진행률(`cardPerformance(card, ref)` — 기준일 `monthRefDate(m)`: 이번 달=오늘, 지난/다음 달=그 달 15일. 카드가 '매월 N일 시작' 커스텀 기간이어도 그 달의 기간을 잡음)을 보여준다. 카드 목록은 **카드 설정(`creditCards`)이 없어도 계좌 유형(`CARD_TYPES`)만으로** 잡아 체크카드·목표 미설정 카드도 나온다(`cardAccounts`), 그 달 결제가 있는 카드만 사용액 순으로.
  - 행을 탭하면 **카드 내역 시트**(`openCardTxSheet(cardId, m)`) — 상단 요약(이 달 결제액·건수 + 실적 진행률·실적 기간·제외 건수) + 그 달 결제 내역(`cardMonthTx` = 달력 월 기준 `expense`/`prepaid_charge`, 최신순). **시트 안에도 ‹ › 월 이동**이 있어 지난달·그전달을 이어서 볼 수 있다. (실적은 카드 기간 기준, 내역 목록은 달력 월 기준 — 각각 라벨로 구분 표기.)
