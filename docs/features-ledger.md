...
- **빠른 입력**: 거래 추가 시트 상단에 **자주 쓴 지출 칩**(원탭으로 유형·카테고리·금액·내용 프리필, `frequentTxTemplates`/`applyTxTemplate`).
...
- **리포트**(`renderStats`): 월 요약·총지출·**CSS 도넛(카테고리)** — 범례에 **[% | 금액] 토글**(`setRepVal`)로 항목별 비율/실제 지출액(원)을 바꿔 봄·6개월 막대·**개인별/공동 지출 분리 바**(`t.user` 집계, `공동` 별도)·**통화별 지출**(해외통화 있을 때)·**전월비 급증 인사이트**·예산 임계 경고·목적별 사용 상위. *Chart.js 미사용 — 순수 CSS 차트.*
