...
### 추가 — 🐱 고양이집 (4단계)
- **최대 3마리 활성 관리 + 주간 미션 + dock 확장/숨김 + 더보기 코인 아이콘**: ① **활성 슬롯**: 홈 탭에서 보유 고양이를 탭해 **집에 내보내기/대기**(최대 3마리, `home/active`). ② **주간 미션**: 이번 주 5일 이상 기록(+20, 진행도)·리포트 확인(+10, `renderStats` 진입 시 `progress` 플래그), KST **월요일 초기화**·멱등 수령(미션 로직 일/주 period 일반화). ③ **전역 dock 3모드**: 접힌 스트립 ↔ **인라인 확장(방 뷰)** ↔ 숨김(▼▲ 토글, 더보기 설정 **고양이집 dock** 켬/숨김). ④ **더보기 고양이집 아이콘 → 은화(코인)** 도트 아이콘. `sw.js` `v3.23.0`.
