...
- **🌱 완료 잔디**(완료 탭 상단, `todoHeatHtml`): 최근 15주 날짜별 완료 개수를 잔디 히트맵(열=주·행=월~일, 4단계 초록)으로. 비반복=완료일(`todoDoneDay`), **반복=완료 시 날짜키 로그 `doneLog/{YYYY-MM-DD}:true`**(toggleTodo가 기록 — lastDoneAt만으론 이력이 안 남아 신설, 기록 시작 시점부터 축적).
