...
- **📅 완료 기준일**(`todoDoneCtxDay`): 할일 **캘린더 탭에서 오늘이 아닌 날짜를 선택한 상태**면 완료 시각(`doneAt`/`lastDoneAt`/스냅샷 `doneAt`)을 **그 날짜 정오(`isoAtNoon`)** 로 기록한다 → 보고 있던 날짜에 완료로 남고 완료 탭·캘린더 점·잔디가 모두 그 날짜 기준(`todoDoneDay`)으로 일치. 그 외 화면(목록·검색 등)은 종전대로 현재 시각. `updatedAt`은 항상 실제 수정 시각.
