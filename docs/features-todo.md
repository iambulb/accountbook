...
- **📅 캘린더 시작 요일**: 월 그리드의 주 시작을 **더보기 → 설정 → '달력 시작 요일'**(월요일 기본 ↔ 일요일)로 바꿀 수 있다 — 가계부 달력과 공용(`weekStartSun`/`calHeadHtml`/`calFirstOffset`, 기기별 localStorage).
...
- **📚 카테고리별 묶음 보기**(`_todoGroup`/`setTodoGroup`/공용 렌더 `todoGroupedHtml`): 할일 목록은 **기본이 카테고리별 묶음** — 카테고리 색점+이름+개수 헤더 카드로 묶고(그룹 순서=카테고리 관리 정의 순, 미분류 마지막), 그룹 안은 마감·우선순위 정렬 유지. 필터 줄 우측 **[카테고리 | 마감순] 토글**로 기존 한 목록(마감순) 전환. **캘린더 날짜 목록도 같은 토글을 따라 카테고리 묶음으로 표시**(`todoGroupedHtml(dayT, ro, sel)` — 완료 기준일 ctxDay 전달 유지). 완료 섹션은 종전대로.
