...
- **매주 / 매월** 반복(`repeat`, 주기 예정 할일). 완료(`toggleTodo`)하면 `done` 을 세우지 않고 **마감일을 다음 회차로 넘김**(`nextDue` — 매주 +7일, 매월 다음 달 같은 날·말일 클램프)·`done` 리셋 → 계속 순환. **매일 하는 습관은 여기가 아니라 내 미션**(알뜰샵 미션 탭·스트릭·리스트 밖)으로 역할 분리 — 반복 필드에서 매일 제외(기존 daily 할일은 하위호환).
