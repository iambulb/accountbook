...
- **🎞️ 걷기 프레임 수는 가변(6·8 등)**: 걷기 애니메이션 장수는 **`PET_SPRITES[id].frames`**(런타임 펫은 RTDB `catalogPets/{id}.frames`)가 단일 소스다. 렌더(`catActorHTML`)가 `--fw = 칸크기 × frames` 필름 폭 + **인라인 `animation-timing-function:steps(frames)`**(CSS `.csprf` 의 `steps(6)`은 기본값일 뿐, 인라인이 덮어씀)로 프레임당 정확히 한 칸 밀어 재생한다. **dev 업로드(`_processPetZip`)는 zip의 실제 east 프레임 장수(최대 12)를 그대로 시트로 합성**하고 `frames` 를 저장하므로, 고등급 펫에 **8프레임**을 넣으면 그대로 부드럽게 재생된다(자연 정렬로 `frame_10<frame_2` 어긋남 방지). *번들 펫 파이프라인(`tools/build_pets.py`)은 아직 6프레임 고정 — 8프레임 번들이 필요하면 그쪽도 확장 필요.*
