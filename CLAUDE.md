...
- **⚠️ `isDock`는 '크기' 전용, '원근' 아님**: `isDock`(`stage.id==='cdStage'`)은 오직 **크기**에만 쓴다(`furnRoomH` base 11 vs 16, `roomH` 폴백 **160** vs 244). **원근 비율(바닥%·`riseMax`·가구 뒤매핑)에는 `isDock` 분기를 절대 넣지 말 것** — 넣는 순간 dock/방이 다시 어긋나 펫이 벽에 못 닿는다(과거 버그: dock `0.61` vs 방 `0.53`).
  - **📐 dock 캠 높이=160px(`.cd-room`)**: 벽 가구 픽셀 높이(`base11×ROOM_H≈15px`)는 방 높이에 **안 비례**한다. 그래서 짧은 dock에선 벽 밴드 위쪽(mount r1=bottom 87%) 가구가 천장에 잘렸다 → **알뜰홈과 같은 아이템 비율(15/160≈9% ≈ 알뜰홈 22/244)로 dock를 160px**로 확장(원근 %는 그대로라 자동 정합). 벽 밴드가 더 높아지거나 벽 가구 `ROOM_H`를 키우면 이 높이도 함께 재검토(필요 높이 ≈ `벽아이템높이 / 0.09`).
