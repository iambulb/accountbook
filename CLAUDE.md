...
- **🐛 반복 버그 방지 체크리스트 (가구·벽지·바닥 만들 때 매번 확인 — 실제로 났던 버그들)**: 아래는 지금까지 **실제로 재발했거나 수정·보완한** 항목들이다. 새 아이템을 추가·수정할 때 이 목록을 훑는다.
  ...
  3. **크기(ROOM_H) ↔ 점유칸(footW/footH) 동기화**: 칸만 줄이고 `ROOM_H`를 그대로 두면 캠에서 안 작아진다(위 "크기·원근"). **벽 가구는 특히 `ROOM_H`가 벽 1칸(`WALL_STEP` 9.3%)에 맞아야** 이웃과 안 겹친다(걸이형 1×1=1.4, 위 "벽꾸미기"). 창문·벽시계·행잉플랜트·모빌 겹침이 이 케이스였다.
...
- **🧱 벽꾸미기(벽 배치)는 바닥과 별개 레이어**: 벽 가구(`ITEM_CATALOG` `wall:true` — 벽난로·창문·벽시계·행잉플랜트·모빌)는 바닥 `placed`가 아니라 방마다의 **`wallPlaced`**(벽 12×4 격자, `r1`=천장…`r4`=바닥선)에 저장한다. 캠에선 `wallPropMarkup`이 벽면(위 46%)에 그린다 — **가로 앵커는 바닥과 동일**(`camAnchorMode`+`--crtx`), **세로는 앵커 3종**(`WALL_ANCHOR`/`wallAnchorOf`): **floor(바닥형=벽난로)**=맨 뒤 바닥 가구와 같은 `bottom:49%`(=`3+1*46`) 바닥선에 서서 바닥에 붙음(항상 맨 아래 행 `wallSnapRow`), **mount(거는형=창문·벽시계)**=벽 밴드 안 `bottom = WALL_MOUNT_BASE(54)+(WALL_ROWS-r)*WALL_MOUNT_STEP(11)`(행=높이), **hang(매다는형=모빌·행잉플랜트)**=천장쪽 `top:%` 앵커로 아래로 늘어짐(`.cr-wall-hang{transform-origin:top}`). **깊이 없음**(z:0=뒤 벽면). ⚠️ **바닥형을 seam(54%)에 두면 뒤 바닥 가구(49%)보다 5% 높아 "붕 뜬다"** — 반드시 바닥선(49%)에 맞춘다. 원근에 `isDock` 분기 금지(크기만). 배치 UI는 **[방꾸미기 | 벽꾸미기] 토글**(`_placeMode`)로 나뉘고 벽도 **방꾸미기와 동일하게 탭·드래그 모두** 지원(`wallPlaceClick` 탭 배치 + **`wallGiDown`(걸린 항목 이동)·`wallPalDown`(팔레트→격자 드래그)** — 바닥의 `giDown`/`palDown` 로직 이식, `wallDropCell`·`#wgdrop` 프리뷰·`_justDragged` 가드 공유), 팔레트는 서로 필터(`isWallItem`). 벽 격자(`.gridwall`)도 12×4 칸 구분선 표시. 인벤토리 소진(`sumPlacedItem`)·친구 스냅샷(`repRoomSnapshot`)·정규화(`util.js normRoom`)에 `wallPlaced`를 함께 반영한다.
  - **⚠️ 벽 가구 렌더 높이는 벽 1칸(`WALL_STEP` 9.3%≈1행)에 맞춘다 — 안 그러면 위·아래 벽칸과 겹친다.** 걸이형 **1×1 벽 가구(창문·벽시계·행잉플랜트·모빌)는 `ROOM_H≈1.4`**(렌더 높이 `16×ROOM_H`px가 1행 ≈ 22px에 들어옴). `2.4~2.6`처럼 크면 1.8행 높이가 돼 이웃을 침범한다(창문·벽시계·행잉플랜트·모빌 겹침 버그의 원인이었다). **벽난로처럼 바닥선(r4)에 서는 2×1 대형**은 예외(크게 둔다). 새 벽 가구 추가 시 `ROOM_H`를 1칸에 맞춰 넣고, 벽에 여러 개 붙여 겹치는지 확인한다.
