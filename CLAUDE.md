...
- **🐛 반복 버그 방지 체크리스트 (가구·벽지·바닥 만들 때 매번 확인 — 실제로 났던 버그들)**: 아래는 지금까지 **실제로 재발했거나 수정·보완한** 항목들이다. 새 아이템을 추가·수정할 때 이 목록을 훑는다.
  ...
  5. **등급·가격**: 은화 구매가는 **등급가 `TIER_PRICE[tier]`**(카탈로그 `price` 아님, 위 "등급·가격" 규칙). 구매가는 항상 `itemBuyPrice`/`wallBuyPrice`/`floorBuyPrice`로 읽는다. **특별(epic)↑ 등급은 자동 랜덤박스 전용**이라 등급만 지정하면 판매 제외+박스풀 편입이 함께 된다.
