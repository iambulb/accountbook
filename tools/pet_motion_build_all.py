...
⚠️ 규칙 생성 범위(2026-07-10 사용자 지침): 새 펫은 기본 7종(idle·sit·belly·eat·drink·yawn·angry)
   ...
   wiggle·bark 등)은 규칙 생성 금지 — zip이 직접 제공할 때만 반영. 신화·한정 16종에 생성했던
   가구 상호작용 6종도 2026-07-10 전량 삭제(m_knead 등 빌더 함수는 참고용으로만 남음).
...
# 🐾 XS 입 — 저해상도(머즐 ~6px)라 가로 ≤2/3(4px) 유지. 하품은 '세로로' 크게 벌려(가이드 ㉡ 과대 개구 금지),
#    하악(hiss)은 4px 송곳니(C). small/eat·drink는 S와 동일(4px).
MOUTHS['XS'] = dict(
 small=['ODDO','.KK.'],
 mid=['OddO','OPPO','.KK.'],
 wide=['OddO','OdPO','OPPO','.KK.'],       # 하품 큰 개구 — 4px 폭 × 세로로 길게(코 아래 열림)
 wide_fang=['OddO','OCPC','OPPO','.KK.'],  # (대형 고양잇과용, XS엔 거의 미사용)
 hiss=['OddO','OCCO','.KK.'],              # 하악 — 송곳니 C
 t_down=['PP','.P'], t_curl=['P.','PP'])
