...
// 픽셀 아트: 문자 매트릭스 → SVG rect(crispEdges) 렌더(px). PNG 미사용(다크모드·캐시·성능 유리).
    ...
    function ddeulFlowerRb(container){ const fl=container&&container.querySelector&&container.querySelector('.fx-ddflower'); if(!fl) return false;
      fl.classList.add('ddflw-rb'); fl.innerHTML=ddeulFlwRbSvg(); return true; }
    ...
    function repRoomSnapshot(){ const h=homeH(); const rooms=h.rooms||[]; const i=Math.min(rooms.length-1, Math.max(0, (h.showRoom!=null?h.showRoom:0)|0)); const r=rooms[i]||rooms[0]||{};
      return { name:r.name||'', emoji:r.emoji||'', wallpaper:r.wallpaper||'default', floor:r.floor||'default', bgfx:r.bgfx||'', placed:r.placed||{}, wallPlaced:r.wallPlaced||{}, active:(r.active||[]).filter(ownsCat), slots:slotCount(), poops:Number(r.poops)||0, changedAt:h.changedAt||'' }; }
    ...
    function useHeldDdeul(){
      ...
      gameRef().transaction(g=>{ g=normalizeGame(g);
        ...
        return g;
      }).then(r=>{ if(r&&r.committed){ runGachaFx('ddeul', res, dup, refund, false, isNew); if(state._sheetRefresh) setTimeout(()=>{ if(state._sheetRefresh) state._sheetRefresh(); }, 50); } else { closeFx(); toast('처리 중이에요 — 잠시 후 다시 시도해 주세요', true); } })
        .catch(function(){ closeFx(); });
    }
    // 미션 수동 수령(완료 판정 후)
    ...
    function rainbowBannerHtml(){
      ...
      return '<div class="gbanner gb-rainbow"><div class="gb-head"><b class="gb-t tier-rainbow">🌈 무지개 · 밤</b></div>'+
        ...
        // 🌈 배너 이미지 아래 — 무지개알 이미지·설명·소모재화(금화 전용)
        '<div class="gb-item"><div class="gb-item-ic">'+rainbowEggSvg({h:52})+'</div>'+
    ...
    function openGachaTen(kind){
      ...
      for(let i=0;i<N;i++){
        ...
        if(kind==='box'){
          ...
          list.push({ id:res.id, tier:res.tier, kind:kind, rainbow:(kind==='egg' && Math.random()<rbUpgradeChance(res.tier)), dup:owned, refund:owned?petDupRefund(res.id):0, isNew:!owned });
          pity=pityNext(pity, isTopTier(res.tier));
        }
      }
    ...
    function openDevBannerManager(){ if(!(typeof isDev==='function'&&isDev())){ toast('개발자 전용'); return; }
      const build=()=>{
        ...
        let h='<div class="note">가챠 배너 미리보기 — 탭별로 확인. <b>펫알</b>=노을 연못·<b>랜덤박스</b>=노을 선물상자(전용 배경 분화). 버튼은 <b>미리보기</b>(1회=강제 전설·10회=연출)로 소모 없음.</div>';
    ...
    function sunsetSceneHtml(mode, variant){
      mode=mode||'banner'; const ck=mode+'|'+(variant||''); if(_sunsetCache[ck]) return _sunsetCache[ck];
      ...
      pond='<div class="pk-pond"><div class="pk-pwater"></div>'+pkoi+lilies+lotuses+pstones+'</div>';
      }
      ...
      _sunsetCache[ck]='<div class="pkscene pk-sunset'+(reveal?' pk-reveal':'')+'" aria-hidden="true">'+
    ...
    function openGacha(kind, free){
      ...
      gameRef().transaction(g=>{
        ...
        return g;
      }).then(r=>{ if(r&&r.committed) runGachaFx(kind, res, dup, refund, false, isNew); else { closeFx(); toast('처리 중이에요 — 잠시 후 다시 시도해 주세요', true); } })   // C4: 중단 시 오버레이 닫고 안내(잠금 해제)
        .catch(function(){ closeFx(); });
    }
    ...
    function openDdeul(free){
      ...
      gameRef().transaction(g=>{ g=normalizeGame(g);
        ...
        return g;
      }).then(r=>{ if(r&&r.committed) runGachaFx('ddeul', res, dup, refund, false, isNew); else { closeFx(); toast('처리 중이에요 — 잠시 후 다시 시도해 주세요', true); } })   // C4
        .catch(function(){ closeFx(); });
    }
    ...
    function useRainbow(kind){
      ...
      gameRef().transaction(g=>{ g=normalizeGame(g);
        ...
        return g;
      }).then(r=>{ if(r&&r.committed) runGachaFx(kind, res, dup, refund, true, isNew); else { closeFx(); toast('처리 중이에요 — 잠시 후 다시 시도해 주세요', true); } })   // C4
        .catch(function(){ closeFx(); });
    }
    let _selItem=null;
    ...
    function catPlaceHtml(){
      ...
      if(wallMode){
        ...
        const grid='<div class="grid12" id="placeGrid" onclick="placeClick(event)">'+items+(items?'':emptyGridHint())+'<div class="gdrop" id="gdrop" hidden></div><div class="wsnap" id="fsnap" hidden></div><div class="wsnaph" id="fsnaph" hidden></div></div>';
    ...
    function fxTap(){
      ...
      if(isEggKind(_fx.kind)){
        ...
        it.innerHTML = _fx.kind==='ddeul' ? ddeulFxHtml(_fx._flwRb?DDEUL_FLW_RB:undefined) : (_fx.rainbow?rainbowEggStage(_fx.stage,{h:150}):eggSvg(_fx.stage,{h:150}));   // 나비 연출(무지개) 시 꽃 무지개 유지
    ...
    function runTenGachaFx(list, opts){ opts=opts||{}; _fxClear(); _fx=null;
      ...
      _fx10={ items:items, order:tenShuffle(items.length), stage:0, busy:true, phase:'nest', ridx:0, preview:!!opts.preview, isBox:isBox,
    ...
    function boxOwned(res){ if(res.type==='floor') return ownsFloor(res.id); if(res.type==='wall') return ownsWall(res.id); if(res.type==='bgfx') return ownsBgfx(res.id);
    ...
    function devPreview(kind, tierId, forceId){
