...
// 픽셀 아트: 문자 매트릭스 → SVG rect(crispEdges) 렌더(px). PNG 미사용(다크모드·캐시·성능 유리).
    ...
    // 펫 화장실(2×2): 모래 트레이. 비운 그릇 수만큼 똥이 쌓임.
    const M_LITTER = [
      "................","................","................","................",
      "..XXXXXXXXXXXX..",".XWSSSSSSSSSSWX.",".XWSSSSSSSSSSWX.",".XWSSSSSSSSSSWX.",
      ".XWSSSSSSSSSSWX.",".XWSSSSSSSSSSWX.",".XWWWWWWWWWWWWX.","..XXXXXXXXXXXX..",
      "................","................","................","................"
    ];
    const M_POOP = [
    ...
    const FURN_PALS={ cushion:{X:'#5b6470',C:'#a9b2be',D:'#868f9c'}, bowl:{X:'#5b6470',W:'#d0d6dd',F:'#d68b4a'}, waterbowl:{X:'#5b6470',W:'#d0d6dd',A:'#5aa9e6'}, tower:{X:'#6f4c28',W:'#c99a5f',P:'#8a6a3f'}, scratcher:{X:'#6f4c28',W:'#c99a5f',P:'#d8b98a'}, litterbox:{X:'#8a8f98',W:'#c9ced6',S:'#e6dcc3'} };
    ...
    const ITEM_CATALOG = [
      ...
      { id:'litterbox', name:'화장실', price:25, size:1, footW:2, footH:2, desc:'비운 그릇 수만큼 똥이 쌓여요. 탭해 치우면 은화!' }
    ];
    ...
    function furnSvg(id, opt){ const M={cushion:M_CUSHION,bowl:M_BOWL,waterbowl:M_WATERBOWL_WATER,tower:M_TOWER,scratcher:M_SCRATCHER,litterbox:M_LITTER}[id]; return pxSvg(M, FURN_PALS[id], opt); }
    ...
    // 무지개(6색) 컨페티는 "한정(limited)" 등급에서만 — 그 외 등급은 등급색+흰색 컨페티(무지개 연출은 특정 조건에서만 나오게).
    function fxConfetti(tier){ const rainbow=['#F04452','#F0883C','#F2C84B','#2FAE7A','#3182F6','#9B6FC8'];
      const cols = tier==='limited' ? rainbow : [tierInfo(tier).color, '#ffffff'];
      let s=''; for(let i=0;i<24;i++){ const x=Math.round(Math.random()*100), r=Math.round(Math.random()*360), del=(Math.random()*0.5).toFixed(2), dur=(1+Math.random()*0.8).toFixed(2); s+='<span class="fx-conf" style="left:'+x+'%;background:'+cols[i%cols.length]+';--r:'+r+'deg;animation-delay:'+del+'s;animation-duration:'+dur+'s"></span>'; } return s; }
    ...
    function fxReveal(){
      ...
      fx.innerHTML='<div class="fx-scrim"></div><div class="fx-reveal tier-'+t.id+'">'+
        ...
        '<div class="fx-confetti">'+fxConfetti(_fx.res.tier)+'</div></div>';
