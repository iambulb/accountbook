    ...
    function petInfoBody(id){
      ...
      return '<div class="gih pi-h">'+piPetHtml(id)+'<b>'+catNameSpan(id,catName(id))+'</b>'+
        ...
        ((PET_SPRITES[id]&&PET_SPRITES[id].clips)?'<div class="pi-cd">모션 해금 — Lv1 기본(유휴·먹기·마시기)'+(al.level>=1?'(해금)':'(잠김)')+' · Lv2 식빵·하품'+(al.level>=2?'(해금)':'(잠김)')+' · Lv4 하악질'+(al.level>=4?'(해금)':'(잠김)')+'</div>':'')+   // 💗 전 등급 애정 모션 해금 안내(Lv1/2/4, 2026-07 사용자 지침 — 신화 미만도 표기)
